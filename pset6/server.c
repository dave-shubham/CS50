...
//Function I added
void split(char *string, char *delim, char *tokens[]);
...
char* indexes(const char* path)
{
    ...
    /*  puts(ep->d_name);
		dp1=opendir(ep->d_name);
		if(dp1!=NULL && strcmp(ep->d_name,".")!=0 && strcmp(ep->d_name,"..")!=0)
		{
			chdir(ep->d_name);
			openDir(dp1);
		}*/
        if(strcasecmp(ep->d_name,"index.html") == 0 )
        {
        //    printf("File found\n");
            flag=0;
            break;
        }
        else if(strcasecmp(ep->d_name,"index.php") == 0)
        {
            flag=1;
            break;
        }
	...
	if(flag == 0)
	{
	    com_path=(char *)malloc(len+12);            //11 characters for /index.html
	    strcpy(com_path,path);
	    strcat(com_path,"/");
	    strcat(com_path,"index.html");
	    return(com_path);
	}
	else if(flag == 1)
	{
	    com_path=(char *)malloc(len+11);            //10 characters for /index.php
	    strcpy(com_path,path);
	    strcat(com_path,"/");
	    strcat(com_path,"index.php");
	    return(com_path);
	}
    return NULL;
}
...
const char* lookup(const char* path)
{
    ...
    if(path != NULL)
    {
        char * string;
        const char ch = '.';
        string=strrchr(path, ch);

        //returns text as per the extension type
        if(strcasecmp(string,".html") == 0)
        {
        //    printf("It's a html request\n");
            return("text/html");
        }
        else if(strcasecmp(string,".css") == 0)
        {
        //    printf("It's a css request\n");
            return("text/css"); 
        else if(strcasecmp(string,".gif") == 0)
        {
        //    printf("It's a gif request\n");
            return("image/gif");
        }
        else if(strcasecmp(string,".ico") == 0)
        {
        //    printf("It's an icon request\n");
            return("image/x-icon");
        }
        else if(strcasecmp(string,".jpg") == 0)
        {
        //    printf("It's a jpeg request\n");
            return("image/jpeg");
        }
        else if(strcasecmp(string,".js") == 0)
        {
        //    printf("It's a java script request\n");
            return("text/javascript");
        }
        else if(strcasecmp(string,".php") == 0)
        {
        //    printf("It's a php request\n");
            return("text/x-php");
        }
        else if(strcasecmp(string,".png") == 0)
        {
        //    printf("It's a png request\n");
            return("image/png");
        }
    }
    return NULL;
...
{
        // TODO
    int len=strlen(line),i,countSpaces=0;
    for(i=0;i<len;i++)
    {
        if(line[i]== ' ')
        {
    ...
    if(countSpaces != 2)            //more than 2 spaces would mean it's not consistent with given rule
    {
    ...
    char string[len+1];
    strcpy(string,line);
    char *tokens[3];        //tokens[0] is method, tokens[1] is absolute path with/without query and tokens[2] is HTTP version
    split(string," ",tokens);

    if(strchr(tokens[1],34) != NULL)           // checking if " is present in absolute address
    {                                          //34 is decimal code for ASCII of "
        //printf("400 Bad Request\n");
        error(400);
        return false;
    }
    if(tokens[1][0]!='/')                       //checking if absolute address starts with /
    {
        //printf("501 Not Implemented\n");
        error(501);
        return false;
    }
    if(strcmp(tokens[0],"GET")!=0)              //checking if METHOD is GET
    {
        //printf("405 Method Not Allowed\n");
        error(405);
        return false;
    }
    char* version = strtok(tokens[2], "\r\n");
			
    if(strcmp(version,"HTTP/1.1") != 0)       //checking version of HTTP is 1.1 along with CRLF
    {
        //printf("505 HTTP Version Not Supported\n");
        error(505);
        return false;
    }
//    abs_path=(char *)malloc(strlen(tokens[1])+1);
    strcpy(abs_path,tokens[1]);
//    printf("%s\n",abs_path);

    if(strchr(tokens[1],63) != NULL)        //If a query is present or even if there is ? at the end of address
    {
        int j=0,len1=strlen(abs_path);
        if(len1-(strchr(abs_path,63)-abs_path) == 1)     //only ? is present
        {
  //          query = (char *)malloc(1);
            query[0]='\0';
        }
        else                                    //query is present after ?
        {
    //        query=(char *)malloc( sizeof(  len-(strchr(line,63)-line)  ) );
            for(i=(strchr(abs_path,63)-abs_path)+1;i<len1;i++,j++)
            {
                query[j]=tokens[1][i];
            }
            query[j]='\0';
...
//Tokenizes the entered string with delimiter "delim"
void split(char *string, char *delim, char *tokens[])
{
    int count = 0;
    char *token;
