...
bool check(const char* word)
{
    ...
    {
        //Checking case of word so that Foo,FOO,fOO, etc are all detected when foo is present in dictionary.
        if(isalpha(word[level]))
        {
            if(isupper(word[level]))
            {
                index=indexOfChar(word[level]+32);
            }
            else
            {
...
{
    // TODO
    root=(trie *)malloc(sizeof(trie));//initialize();
    if(root == NULL)
    {
...
int insert(const char * word)
{
    trie *traverse;
    traverse=root;
    int level=0,len=strlen(word),index,i;
    for(level=0;level<len;level++)
    {
        index=indexOfChar(word[level]);
        //printf("%d\t%c\n",index,word[level]);
        if(traverse->alpha[index]==NULL)
        {
            traverse->alpha[index]=(trie *)malloc(sizeof(trie));//initialize();
            if(traverse->alpha[index] == NULL)
            {
                printf("Could not allocate node\n");
                return(0);
            ...
            for(i=0;i<27;i++)
            {
                traverse->alpha[index]->alpha[i]=NULL;
            }
            traverse->alpha[index]->isEOW=0;
        }
        traverse=traverse->alpha[index];
    }
